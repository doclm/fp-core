...
var Memory = require('./agent').Memory;
var MemoryOfAgent = require('./agent').MemoryOfAgent;
...
//        var positions = [{x: x, y: y}];
        positions.forEach(function(position) {
            ...
            if (agents && agents.length > 0) {
                agents.forEach(function(otherAgent) {
                    if (otherAgent.id != agent.id && otherAgent.age > 0 && otherAgent.alive && ! world.isExitPoint(otherAgent.x, otherAgent.y))
