...
var ModuleManager = require('./module-manager').ModuleManager;
                ...
                if (Lifecycle.currentWorld.isExitPoint(agent.x, agent.y)) {
                    ...
                    if (ModuleManager.currentModule.id = 'Default')
                        if (typeof(FiercePlanet) !== "undefined")
                            FiercePlanet.GameState.currentProfile.currentWorldExpired++;
                }
            }
        }
...
//                    if (agent.culture == DefaultCultures.CITIZEN_AGENT_TYPE)
        ...
        if (! Lifecycle.currentWorld.playIndefinitely) {
            console.log(Lifecycle.currentWorld.expiredAgents.length)
    ...
    this._startAgents = function () {
        ...
        this.inPlay = true;
        this.interval = (! _.isUndefined(Lifecycle.currentWorld.interval) && Lifecycle.currentWorld.interval > 0 ?
            Lifecycle.currentWorld.interval : this.interval)
    ...
    this.processAgentsInBrowser = function() {
        if (Lifecycle.inPlay) {
            if (typeof window != 'undefined') {
                ...
                Lifecycle.agentTimerId = setTimeout(Lifecycle.processAgentsInBrowser, this.interval);
                Lifecycle.processAgents();
            }
        }
    }
